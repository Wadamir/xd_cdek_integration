...
class ModelExtensionShippingCdek extends Model
{
    ...
    function __construct($registry)
    {
        ...
        $api_account = $this->config->get('cdek_login');
        $api_secure = $this->config->get('cdek_password');

        $this->api = new cdek_integrator($api_account, $api_secure);
    }

    private function cdekLog($message)
    {
        $log = new Log('cdek.log');
        $log->write($message);
    }
    ...
    function getQuote($address)
    {
        ...
        $this->cdekLog('DEBUG: getQuote() вызван');
        $this->cdekLog('DEBUG: cdek_store = ' . json_encode($this->config->get('cdek_store')));
        $this->cdekLog('DEBUG: config_store_id = ' . $this->config->get('config_store_id'));
        $this->cdekLog('DEBUG: currency RUB exists = ' . ($this->currency->getId('RUB') ? 'YES' : 'NO'));
        $this->cdekLog('DEBUG: cdek_city_from_id = ' . $this->config->get('cdek_city_from_id'));
        ...
        if (!is_array($this->config->get('cdek_store')) || !in_array($this->config->get('config_store_id'), $this->config->get('cdek_store'))) {
            ...
            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: не выбран магазин!');
            }
        ...
        if (!$this->currency->getId('RUB')) {
            ...
            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: в системе не найдена валюта "RUB"!');
            }
        ...
        if (!$city_from = $this->config->get('cdek_city_from_id')) {
            ...
            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: не выбран город отправки!');
            }
        ...
        // DEBUG: логирование адреса доставки
        $this->cdekLog('DEBUG: address = ' . json_encode($address));

        $this->auth_data = $this->getInfo()->getAuthToken();

        $this->cdekLog('DEBUG: auth_data type = ' . gettype($this->auth_data));
        if (is_array($this->auth_data)) {
            $this->cdekLog('DEBUG: auth_data keys = ' . json_encode(array_keys($this->auth_data)));
            if (isset($this->auth_data['access_token'])) {
                $this->cdekLog('DEBUG: access_token exists = YES (length: ' . strlen($this->auth_data['access_token']) . ')');
            } else {
                $this->cdekLog('DEBUG: access_token exists = NO');
            }
        }
        ...
        $this->cdekLog('DEBUG: status = ' . ($status ? 'TRUE' : 'FALSE') . ' перед проверкой веса');
        $this->cdekLog('DEBUG: weight = ' . $weight . ', min = ' . $min_weight . ', max = ' . $max_weight);

        if ($status && (($min_weight > 0 && $weight < $min_weight) || ($max_weight > 0 && $weight > $max_weight))) {

            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: превышены ограничения по весу!');
            }
        ...
        if ($this->config->get('cdek_log')) {
            $this->cdekLog('СДЭК: вес заказа ' . $weight);
        }
        ...
        $this->cdekLog('DEBUG: total = ' . $total . ', min = ' . $min_total . ', max = ' . $max_total);

        if ($status && (($min_total > 0 && $total < $min_total) || ($max_total > 0 && $total > $max_total))) {

            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: превышены ограничения по стоимости!');
            }
        ...
        $this->cdekLog('DEBUG: перед основной логикой - status = ' . ($status ? 'TRUE' : 'FALSE') . ', to_data = ' . (isset($to_data) && $to_data ? 'YES' : 'NO') . ', city = ' . (isset($address['city']) ? $address['city'] : 'EMPTY'));
        ...
        if ($to_data && $address['city']) {
            ...
            if (is_array($cdek_cities)) {
                ...
                if ($this->config->get('cdek_city_ignore')) {
                    ...
                    $city_ignore = preg_split('#\s*,\s*#', $this->config->get('cdek_city_ignore'));
                ...
                if ($this->config->get('cdek_use_region_russia')) {
                    ...
                    $this->cdekLog('DEBUG: Ищу регион "' . $address['zone'] . '" в таблице cdek_city');
                    ...
                    $this->cdekLog('DEBUG: Найдено строк в cdek_city: ' . $zoneQuery->num_rows);
                    if ($zoneQuery->num_rows) {
                        $this->cdekLog('DEBUG: regionName из БД: ' . $zoneQuery->row['regionName']);
                        $newAvailable = array();

                        foreach ($available as $key => $available_value) {
                            if (!isset($available_value['region'])) {
                                $this->cdekLog('DEBUG: Город ' . $available_value['city'] . ' не имеет region!');
                                continue;
                            }

                            $this->cdekLog('DEBUG: Сравниваю регионы: API=' . $available_value['region'] . ' vs DB=' . $zoneQuery->row['regionName']);
                        ...
                        if (count($newAvailable)) {
                            ...
                            if ($this->config->get('cdek_log')) {
                                $this->cdekLog('СДЭК WARNING: не удалось отфильтровать города по региону!');
                                $this->cdekLog('СДЭК WARNING: город: ' . $address['city']);
                                $this->cdekLog('СДЭК WARNING: регион: ' . $address['zone']);
                            }
                        }
                    } else {
                        if ($this->config->get('cdek_log')) {
                            $this->cdekLog('СДЭК WARNING: не найден регион ' . $address['zone']);
                        }
                    }
                }

                if ($this->config->get('cdek_use_postcode') && $address['postcode']) {
                    $this->cdekLog('DEBUG: Фильтрую по почтовому коду: ' . $address['postcode']);
                    $newAvailable = array();

                    foreach ($available as $key => $available_value) {
                        if (!empty($available_value['postal_codes'])) {
                            $postCodeArray = $available_value['postal_codes'];
                            $this->cdekLog('DEBUG: Город ' . $available_value['city'] . ' коды: ' . implode(',', $postCodeArray));

                            if ($postCodeArray && in_array($address['postcode'], $postCodeArray)) {
                                $newAvailable[] = $available_value;
                                $this->cdekLog('DEBUG: Код найден! Город добавлен в результаты');
                            }
                        } else {
                            $this->cdekLog('DEBUG: Город ' . $available_value['city'] . ' - postal_codes пуст');
                        }
                    }
                    ...
                    if (count($newAvailable)) {
                        ...
                        if ($this->config->get('cdek_log')) {
                            $this->cdekLog('СДЭК WARNING: не удалось отфильтровать города по почтовому коду!');
                            $this->cdekLog('СДЭК WARNING: город: ' . $address['city']);
                            $this->cdekLog('СДЭК WARNING: индекс: ' . $address['postcode']);
                            $this->cdekLog('СДЭК WARNING: регион: ' . $address['zone']);
                        }
                    }
                }
                ...
                if ($count = count($available)) {
                    ...
                    if ($this->config->get('cdek_log')) {
                        $this->cdekLog('СДЭК INFO: город (покупатель): ' . $address['city']);
                        $this->cdekLog('СДЭК INFO: регион (покупатель): ' . $address['zone']);
                        ...
                        if ($zoneQuery_debug->num_rows) {
                            $this->cdekLog('СДЭК INFO: город (определен): ' . $zoneQuery_debug->row['cityName'] . ', ' . $zoneQuery_debug->row['regionName']);
                        } else {
                            $this->cdekLog('СДЭК INFO: город (определен): ' . $city_to);
                        }
                    }
                    ...
                    if (!class_exists('CalculatePriceDeliveryCdek')) {
                        ...
                        if ($this->config->get('cdek_log')) {
                            $this->cdekLog('СДЭК: class CalculatePriceDeliveryCdek not found!');
                        }
                    ...
                    $cdek_default_param = $this->config->get('cdek_default_param');
                    ...
                    if ($cdek_default_param['use']) {
                        ...
                        foreach ($products as $product) {
                            ...
                            if ((int)$weight == 0) {
                                if ($this->config->get('cdek_log')) {
                                    $this->cdekLog('СДЭК: не заполнен вес у товара!');
                                }
                            ...
                            if ((int)$length == 0) {
                                if ($this->config->get('cdek_log')) {
                                    $this->cdekLog('СДЭК: не заполнена длина у товара!');
                                }
                            ...
                            if ((int)$width == 0) {
                                if ($this->config->get('cdek_log')) {
                                    $this->cdekLog('СДЭК: не заполнена ширина у товара!');
                                }
                            ...
                            if ((int)$height == 0) {
                                if ($this->config->get('cdek_log')) {
                                    $this->cdekLog('СДЭК: не заполнена высота у товара!');
                                }
                        ...
                        if ($status) {
                            ...
                            foreach ($this->config->get('cdek_custmer_tariff_list') as $key => $tariff_info) {
                                ...
                                if (($min_weight > 0 && $weight < $min_weight) || ($max_weight > 0 && $weight > $max_weight)) {
                                    ...
                                    if ($this->config->get('cdek_log')) {
                                        $this->cdekLog('СДЭК: Тариф «' . $tariff_title . '» превышены ограничения по весу!');
                                    }
                                ...
                                if (($min_total > 0 && $total < $min_total) || ($max_total > 0 && $total > $max_total)) {
                                    ...
                                    if ($this->config->get('cdek_log')) {
                                        $this->cdekLog('СДЭК: Тариф «' . $tariff_title . '» превышены ограничения по стоимости!');
                                    }
                            ...
                            if (!$tariff_list) {
                                ...
                                if ($this->config->get('cdek_log')) {
                                    $this->cdekLog('СДЭК: Не сформирован список тарифов для текущей географической зоны!');
                                }
                            ...
                            if ($status) {
                                ...
                                if (!empty($results)) {
                                    ...
                                    foreach ($results as $shipping_info) {
                                        ...
                                        if (array_key_exists($shipping_info['tariffId'], $cdek_tariff_list)) {
                                            ...
                                            if (!$this->config->get('cdek_empty_address') && trim($address['address_1']) == '' && in_array($tariff_info['mode_id'], array(1, 3))) {
                                                ...
                                                if ($this->config->get('cdek_log')) {
                                                    $this->cdekLog('СДЭК: пустой адрес доставки для тарифа ' . $shipping_info['tariffId']);
                                                }
                                    ...
                                    if ($this->config->get('cdek_log')) {
                                        $this->cdekLog('СДЭК: нет результатов для вывода!');
                                    }
                                }
                            }
                        }
                    ...
                    if ($this->config->get('cdek_log')) {
                        $this->cdekLog('СДЭК: не определен подходящий город!');
                    }
                }
            } else {

                if ($this->config->get('cdek_log')) {
                    $this->cdekLog('СДЭК: город доставки не определен!');
                }
            }
        } else {

            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: город доставки не найден!');
            }
        }
    ...
    private function getResult(CalculatePriceDeliveryCdek &$calc, $total = 0)
    {
        ...
        if ($calc->calculate() === true) {
            ...
            if (!$this->config->get('cdek_cache_on_delivery') || !array_key_exists('cashOnDelivery', $response) || ($this->config->get('cdek_cache_on_delivery') && (float)$response['cashOnDelivery'] && $total >= (float)$response['cashOnDelivery'])) {
                $result = $response;
            }
        } else {
            ...
            if (isset($error['error']) && !empty($error) && $this->config->get('cdek_log')) {
                foreach ($error['error'] as $error_info) {
                    $this->cdekLog('СДЭК: ' . $error_info['text']);
                }
            }
        }
    ...
    private function getPVZList($city_id, $weight = 0, $type)
    {
        ...
        if (isset($pvz_list_data)) {
            ...
            foreach ($pvz_list_data as $pvz_info) {
                ...
                if ($use_limit && isset($pvz_info->WeightLimit)) {
                    ...
                    if (($min_weight > 0 && $weight < $min_weight) || ($max_weight > 0 && $weight > $max_weight)) {
                        ...
                        if ($this->config->get('cdek_log')) {
                            $this->cdekLog('СДЭК: превышены ограничения по весу для ПВЗ ' . $pvz_info['name'] . ' по адресу: ' . $pvz_info['location']['address'] . '!');
                        }
            ...
            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК: В выбранном городе ПВЗ отсутствуют!');
            }
        }
    ...
    private function getURL($url, response_parser $parser, $data = array())
    {
        $ch = curl_init();
        if ($this->config->get('cdek_log')) {
            $this->cdekLog('СДЭК url запроса: ' . $url);
        }
        ...
        if (!empty($data)) {
            if ($this->config->get('cdek_log')) {
                $this->cdekLog('СДЭК передаваемые данные: ' . $data);
            }
        ...
        $out = curl_exec($ch);
        if ($this->config->get('cdek_log')) {
            $this->cdekLog('СДЭК ответ от api: ' . $out);
        }
        $parser->setData($out);
