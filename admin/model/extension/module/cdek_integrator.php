...
class ModelExtensionModuleCdekIntegrator extends Model
{
    ...
    public function addDispatch($data = array())
    {
        ...
        if (!empty($data['orders'])) {
            ...
            $sql .= "`reason_id` = 0, "; // Additional status
        ...
        if ($dispatch_info = $this->getDispatchInfo($order_id)) {
            ...
            if (!empty($data['call']['good'])) {
                ...
                foreach ($data['call']['good'] as $call_info) {
                    ...
                    $sql .= "`date_delivery` = '" . $this->db->escape($call_info['date_delivery']) . "';";
    ...
    public function install()
    {
        ...
        /* Request to create a server IP record to prevent errors when changing
    order status according to the CDEK status mapping configured in shipment settings */
        $host = gethostname();
        ...
        $sql .= "`order_id` int(11) NOT NULL, ";
        $sql .= "`date` int(10) NOT NULL, ";
        $sql .= "`date_delivery` int(10) NOT NULL, ";
        $sql .= "KEY `order_id` (`order_id`) ";
        $sql .= ") ENGINE=MyISAM DEFAULT CHARSET=utf8;";
    ...
    public function getDispatchToSync()
    {
        ...
        $exchange_interval = 21600; // 6 hours

        $filter_statuses = array(
            4,      // Delivered
            5,      // Not delivered, returned
            16,     // Returned to sender warehouse
            2       // Deleted
        );
