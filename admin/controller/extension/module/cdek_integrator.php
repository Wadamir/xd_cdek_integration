...
class ControllerExtensionModuleCdekIntegrator extends Controller
{
    ...
    public function index()
    {
        ...
        $rdata['heading_title'] = $this->language->get('heading_title');
        ...
        if (!$this->new_application) {
            ...
            $rdata['attention'] = $this->language->get('text_attention_setup_required');
        }
        ...
        foreach ($results as $dispatch_info) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('extension/module/cdek_integrator/dispatchView', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $dispatch_info['order_id'], true)
            );
            ...
            $rdata['dispatches'][] = array(
                ...
                'sync'                    => $this->url->link('extension/module/cdek_integrator/dispatchSync', 'user_token=' . $this->session->data['user_token'] . '&target=list&order_id=' . $dispatch_info['order_id'], true),
                'action'                => $action
            );
        }

        $rdata['dispatch_list'] = $this->url->link('extension/module/cdek_integrator/dispatch', 'user_token=' . $this->session->data['user_token'], true);

        $rdata['order'] = $this->url->link('extension/module/cdek_integrator/order', 'user_token=' . $this->session->data['user_token'], true);
        $rdata['option'] = $this->url->link('extension/module/cdek_integrator/option', 'user_token=' . $this->session->data['user_token'], true);
    ...
    public function getMonth($number)
    {
        ...
        $months = array(
            1  => 'text_month_january',
            2  => 'text_month_february',
            3  => 'text_month_march',
            4  => 'text_month_april',
            5  => 'text_month_may',
            6  => 'text_month_june',
            7  => 'text_month_july',
            8  => 'text_month_august',
            9  => 'text_month_september',
            10 => 'text_month_october',
            11 => 'text_month_november',
            12 => 'text_month_december'
        );

        if (isset($months[(int)$number])) {
            return $this->language->get($months[(int)$number]);
        }
    ...
    public function createOrder()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateOrderFrom()) {
            ...
            if (isset($response) && empty($response['requests']['errors'])) {
                ...
                $this->error['warning'][] = $this->language->get('error_cdek_no_server_response');
            }
            ...
            if (!empty($cdek_orders)) {
                ...
                if (!isset($cdek_orders_post['status_id'])) {
                    ...
                    $status_history[] = array(
                        ...
                        'description'       => $this->language->get('text_status_accepted'),
                ...
                $this->session->data['success'] = $this->language->get('text_success_order_shipped');
    ...
    public function option()
    {
        ...
        $rdata['entry_size'] = $this->language->get('entry_size');
        $rdata['entry_shipment_point_default'] = $this->language->get('entry_shipment_point_default');
        $rdata['entry_size'] = $this->language->get('entry_size');
        $rdata['entry_seller_inn'] = $this->language->get('entry_seller_inn');
        $rdata['entry_seller_ownership'] = $this->language->get('entry_seller_ownership');
        $rdata['entry_seller_address'] = $this->language->get('entry_seller_address');
        $rdata['entry_package_default'] = $this->language->get('entry_package_default');
        $rdata['entry_size_help'] = $this->language->get('entry_size_help');
        ...
        $rdata['token'] = isset($this->session->data['user_token']) ? $this->session->data['user_token'] : '';
    ...
    public function dispatchView()
    {
        ...
        if ($dispatch_info) {
            ...
            $rdata['heading_title'] = $this->language->get('heading_title_order_details');
            ...
            if ($tariff_info) {
                ...
                $rdata['tariff'] = array('title' => '<span class="error">' . $this->language->get('error_tariff_not_defined') . '</span>');
            }
            ...
            if (!empty($call_history_good)) {
                ...
                foreach ($call_history_good as $call_good_info) {
                    ...
                    $rdata['call_history']['good'][] = array(
                        ...
                        'date_delivery'    => $this->formatDate($call_good_info['date_delivery'])
                    );
                }
            }
            ...
            if ($weight_class_info) {
                ...
                $rdata['weight_class'] = $this->language->get('text_grams');
            }
            ...
            if ($length_class_info) {
                ...
                $rdata['length_class'] = $this->language->get('text_centimeters');
            }
            ...
            $rdata['success'] = sprintf($this->language->get('error_dispatch_not_found'), $this->request->get['order_id']);
            $this->response->redirect($this->url->link('extension/module/cdek_integrator/dispatch', 'user_token=' . $this->session->data['user_token'], 'SSL'));
        }
    ...
    public function dispatchPrint()
    {
        ...
        if ($this->isAjax()) {
            ...
            if ($this->new_application || empty($this->request->get['order_id'])) {
                $json['status'] = 'error';
                $json['message'] = $this->language->get('error_receipt_load_failed');
            }
        } else {
            ...
            if (!empty($pdf_info['requests']['errors'])) {
                ...
                if (!empty($pdf_receipt['requests']['errors'])) {
                    ...
                    if ($pdf != '') {
                        ...
                        $this->session->data['success'] = sprintf($this->language->get('text_success_receipt_loaded'), $this->request->get['order_id']);
                    } else {
                        $this->session->data['warning'] = $this->language->get('error_receipt_load_retry');
                    }
                }
            }
        } else {

            $this->session->data['warning'] = sprintf($this->language->get('error_order_not_found_in_cdek'), $this->request->get['order_id']);
        }
    ...
    public function dispatchDelete()
    {
        ...
        if ($dispatch_info && ($dispatch_info['status_id'] == 'ACCEPTED') || $dispatch_info['status_id'] == 'INVALID') { // Can only delete a new order
            ...
            if ($forced || empty($response['requests']['errors'])) {
                ...
                $this->session->data['success'] = sprintf($this->language->get('text_success_order_deleted'), $dispatch_info['number']);
                $this->response->redirect($this->url->link('extension/module/cdek_integrator/dispatch', 'user_token=' . $this->session->data['user_token'], 'SSL'));
            } else {
    ...
    public function dispatchSync()
    {
        ...
        if ($dispatch_info) {
            ...
            if ((!empty($update)) && array_key_exists($this->request->get['order_id'], $update)) {
                ...
                $this->session->data['success'] = sprintf($this->language->get('text_success_order_synced'), $this->request->get['order_id'], $this->language->get('text_sync_date'), $this->formatDate(time(), TRUE, FALSE));
            } else {
                $this->session->data['warning'] = sprintf($this->language->get('error_order_shipped_incorrect'), $this->request->get['order_id']);
            }
        } else {
            $this->session->data['warning'] = sprintf($this->language->get('error_order_not_found_in_cdek'), $this->request->get['order_id']);
        }
    ...
    private function prepareCountry($name = '')
    {
        ...
        } elseif (in_array($name, array('китайская народная республика', 'china'))) {
            $countries[] = 'китай (кнр)';
        } else {
            $countries[] = $name;
        }
    ...
    private function prepareRegion($name = '')
    {
        ...
        if (in_array($parts[0], array('московская', 'москва'))) {
            ...
            $regions[] = $parts[1];
        } elseif (in_array($parts[0], array('киев', 'киевская'))) { // Ukraine
            $regions[] = 'киевская';
            $regions[] = 'киев';
        } elseif (in_array($parts[0], array('винница', 'винницкая'))) { // Ukraine
            $regions[] = 'винница';
            $regions[] = 'винницкая';
        } elseif (in_array($parts[0], array('днепропетровск', 'днепропетровская'))) { // Ukraine
            ...
            if (!empty($post['cdek_orders'])) {
                ...
                foreach ($order_info['package'] as $package_id => $package_info) {
                    ...
                    if (!empty($package_info['item'])) {
                        ...
                        $this->error['cdek_orders']['package'][$package_id]['warning'] = $this->language->get('error_package_item_list_empty');
                    }
                }
                ...
                if (!$order_info['tariff_id'] || !isset($tariff_list[$order_info['tariff_id']])) {
                    ...
                    if (in_array($tariff_info['mode_id'], array(1, 3))) { // Door-to-Door, Pvz-to-Door
    ...
    public function cron()
    {
        ...
        if (!isset($this->setting['order_status_rule'])) {
            $this->log->write($this->language->get('log_status_rules_not_set'));
            return;
        }
        ...
        foreach ($orders as $order) {
            ...
            if (isset($info['requests'][0]['errors'])) {
                $error_msg = isset($info['requests'][0]['errors'][0]['message']) ? $info['requests'][0]['errors'][0]['message'] : 'Unknown error';
                echo "Error for dispatch " . $dispatch_number . ": " . $error_msg . PHP_EOL;
                continue;
            }
            if (!isset($dispatches[$dispatch_number])) {
                echo "WARNING: Not issues dispatch " . $dispatch_number . PHP_EOL;
                continue;
            }
            ...
            if ($dispatch['status_id'] == $status_id) {
                echo "Order with dispatch " . $dispatch_number . " not changed" . PHP_EOL;
                continue;
            }
    ...
    public function logWrite()
    {
        if ($this->config->get('cdek_log')) {
            $this->log->write($this->language->get('log_cdek_request_url'));
            $this->log->write(print_r($this->api->curl_url, 1));
            $this->log->write($this->language->get('log_cdek_request_data'));
            $this->log->write(print_r($this->api->curl_data, 1));
            $this->log->write($this->language->get('log_cdek_response'));
        ...
        if (isset($auth_token['access_token'])) {
            ...
            foreach ($countries as $country) {
                ...
                for ($i = 0;; $i++) {
                    ...
                    // echo "Updated cities: " . 1000*$i . ". Update in progress./n";
