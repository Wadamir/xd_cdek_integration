				...
				<table class="form table">
					<tbody>
						...
						<?php if (!empty($tariff['mode_id'])) { ?>
							<?php if (in_array($tariff['mode_id'], [1, 3])) { ?>
								<tr>
									<td>Улица</td>
						...
						<tbody>
							<?php foreach ($schedule as $row => $attempt_info) { ?>
								<tr class="item-slider schedule row-<?php echo $attempt_info['delay'] != '' ? 'warning' : 'status'; ?>">
									...
									<td>
										<span class="month"><?php echo $attempt_info['date']; ?></span> c <?php echo $attempt_info['time_beg']; ?> до <?php echo $attempt_info['time_end']; ?><?php if ($attempt_info['delay'] != '') { ?> (задержан)<?php } ?><?php if ($attempt_info['show_more']) { ?> <a class="js">Подробнее</a><?php } ?>
									</td>
								</tr>
								<?php if ($attempt_info['show_more']) { ?>
									<tr class="item-slider schedule children  row-<?php echo $attempt_info['delay'] != '' ? 'warning' : 'status'; ?>" style="display: none;">
										<td></td>
										<td>
